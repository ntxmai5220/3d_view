...
class AddImageBS extends StatelessWidget {
  ...
  openByAndroid(BuildContext context) async {
    ...
    if (isInstalled) {
      var reuslt = await LaunchApp.openApp(
          androidPackageName: AppConstants.cameraPackage, openStore: false);

      if (reuslt == 1) {
        uploadApfterCapture(context);
      } //open to take photo
    } else {
      ...
      if (result.type == ResultType.fileNotFound) {
        ...
        if (download && statuses[Permission.storage]!.isGranted) {
          ...
          if (isSuccess) {
            var result2 = await OpenFile.open(savePath);
            if (result2.type == ResultType.done) {
              uploadApfterCapture(context);
            } else {}
            // openCameraPackage(context, savePath: savePath);
          }
        } else {
          pickImage(context, source: ImageSource.camera);
        }
  ...
  uploadApfterCapture(BuildContext context) async {
    await Future.delayed(const Duration(seconds: 2));
    uploadImage(context);
  }

  // openCameraPackage(BuildContext context, {required String savePath}) async {
